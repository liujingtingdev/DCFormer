...
class ChunkedGenerator:
    def __init__(self, batch_size, cameras, poses_3d, poses_2d, poses_2d_crop, valid_frame,
        ...
        if train == True:
            for key in poses_2d.keys():
                ...
                keys = np.tile(np.array(key).reshape([1,3]), (len(bounds_low), 1))
        ...
        self.train_img_path = "dataset/mpi_train_crop"
        self.test_img_path = "dataset/mpi_test_crop"
        self.train_depth_path = "dataset/mpi_train_depth"
        self.test_depth_path = "dataset/mpi_test_depth"
    ...
    def get_batch(self, seq_i, start_3d, end_3d, flip, reverse):
        if self.train == True:
            ...
            depth_dir = self.train_depth_path
            subject, seq, cam_index = seq_i
            seq_name = (subject, seq, cam_index)
            subdir = os.path.join(subject, seq, 'imageFrames', f'video_{int(cam_index)}')
            img_dir = os.path.join(img_dir, subdir)
            depth_dir = os.path.join(depth_dir, subdir)
        else:
            img_dir = self.test_img_path
            depth_dir = self.test_depth_path
            seq_name = seq_i[0]
            subdir = os.path.join(seq_name, 'imageSequence')
            img_dir = os.path.join(img_dir, subdir)
            depth_dir = os.path.join(depth_dir, subdir)

        file_path = os.path.join(img_dir, f'{int(start_3d)+1:06d}.jpg')
        depth_path = os.path.join(depth_dir, f'{int(start_3d)+1:06d}.jpg')

        self.batch_img = cv2.imread(
            file_path, cv2.IMREAD_COLOR | cv2.IMREAD_IGNORE_ORIENTATION
            )
        self.batch_depth = cv2.imread(
            depth_path, cv2.IMREAD_GRAYSCALE | cv2.IMREAD_IGNORE_ORIENTATION
            )
        ...
        if pad_left_2d != 0 or pad_right_2d != 0:
            ...
            self.batch_2d_crop = np.pad(seq_2d_crop[low_2d:high_2d], ((pad_left_2d, pad_right_2d), (0, 0), (0, 0)), 'edge')
        else:
            self.batch_2d = seq_2d[low_2d:high_2d]
            self.batch_2d_crop = seq_2d_crop[low_2d:high_2d]

        # self.batch_img = (self.batch_img[:, :, ::-1] / 255.0 - self.mean) / self.std
        # (1, 17, 2) (256, 192, 3)

        # if flip:
        #     self.batch_img = self.batch_img[:, ::-1].copy()
        #     self.batch_2d[:, :, 0] *= -1
        #     self.batch_2d[:, self.kps_left + self.kps_right] = self.batch_2d[ :,
        #                                                           self.kps_right + self.kps_left]
        #     self.batch_2d_crop[:, :, 0] = 192 - self.batch_2d_crop[:, :, 0] - 1
        #     self.batch_2d_crop[:, self.kps_left + self.kps_right] = self.batch_2d_crop[ :,
        #                                                             self.kps_right + self.kps_left]
        # if self.train:
        #     for j in range(self.batch_2d.shape[1]):
        #         cv2.circle(self.batch_img, (int(self.batch_2d_crop[0,j,0]), int(self.batch_2d_crop[0,j,1])), 3, (255, 0, 0), -1)
        #         retval = cv2.imwrite("./demo/" + '{:06d}.jpg'.format(start_3d + 1), self.batch_img)
        ...
        if self.train == True:
            ...
            if self.poses_3d is None and self.cameras is None:
                ...
                return np.zeros(9), self.batch_3d.copy(), self.batch_2d.copy(), self.batch_2d_crop.copy(), self.batch_img.copy(), self.batch_depth.copy(), seq, subject, int(cam_index)
            elif self.poses_3d is None:
                return self.batch_cam, None, self.batch_2d.copy(),seq, subject, int(cam_index)
            else:
                return self.batch_cam, self.batch_3d.copy(), self.batch_2d.copy(), self.batch_2d_crop.copy(), self.batch_img.copy(), self.batch_depth.copy(), seq, subject, int(cam_index)
        else:
            if self.MAE:
                return np.zeros(9), self.batch_2d.copy(), seq_name, None, None
            else:
                return np.zeros(9), self.batch_3d.copy(), self.batch_2d.copy(), self.batch_2d_crop.copy(), self.batch_img.copy(), self.batch_depth.copy(), seq_name, None, None
