...
from model.DGPose import DepthGuidedPose
...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if False:
            ...
            if split == "train":
                batch_cam, gt_3D, input_2D, input_2D_crop, img, depth, seq, subject, scale, bb_box, cam_ind = data
            else:
                batch_cam, gt_3D, input_2D, input_2D_crop, img, depth, seq, scale, bb_box = data

            [input_2D, input_2D_crop, img, depth, gt_3D] = get_varialbe(split, [input_2D, input_2D_crop, img, depth, gt_3D], torch.device(0))
            ...
            if opt.test_augmentation and split =='test':
                input_2D, output_3D, output_3D_VTE = input_augmentation(img, depth, input_2D.squeeze(), input_2D_crop.squeeze(), model, joints_left, joints_right)
            else:
                output_3D, joint_depth, s = model(img, depth, input_2D.squeeze(), input_2D_crop.squeeze())
            ...
            if split == 'train':
                # ****************
                output_3D_single = output_3D_single/1000.0
                out_target_single = out_target_single/1000.0
                loss = mpjpe_cal(output_3D_single, out_target_single)

                loss_d = loss_depth(joint_depth, out_target_single[...,-1:].squeeze(1), s)
                loss = loss+loss_d*0.00001
            elif split == 'test':
        ...
        if split == 'train':
            ...
            if not opt.MAE:
                ...
                if opt.refine:
                    ...
                    pred_out[:,:,14,:] = 0
                    joint_error = mpjpe_cal(pred_out, out_target_single*1000).item()
    ...
    if split == 'train':
        if opt.MAE:
            ...
            return loss_all['loss'].avg*1000, error_sum.avg
    elif split == 'test':
        if opt.MAE:
        ...
        if opt.refine:
            ...
            if opt.train == 0:
                for seq_name in data_inference.keys():
                    ...
                    print(seq_name,len(data_inference[seq_name]))
                    print(seq_name,data_inference[seq_name].shape)
...
def input_augmentation_MAE(input_2D, model, joints_left, joints_right, mask, spatial_mask=None):
    ...
    output_2D_flip = model(img[:, 1], depth[:, 1], input_2D[:, 1], input_2D_crop[:, 1])
...
def input_augmentation(img, depth, input_2D, input_2D_crop, model, joints_left, joints_right):
    output_3D_flip, _, _ = model(img[:, 1], depth[:, 1], input_2D[:, 1], input_2D_crop[:, 1])

    output_3D_flip[:, 0] *= -1

    output_3D_flip[:, :, :, joints_left + joints_right] = output_3D_flip[:, :, :, joints_right + joints_left]

    output_3D_non_flip, _, _ = model(img[:, 0], depth[:, 0], input_2D[:, 0], input_2D_crop[:, 0])
...
if __name__ == '__main__':
    ...
    if opt.reload == 1:
        if opt.backbone == 'hrnet_32':
            state_dict = torch.load('./checkpoint/model_uncer_DGPose_w32_v2b_1/no_refine_60_2939.pth')
    ...
    {
        "params":
            [p for n, p in model.Lifting_net.named_parameters()
