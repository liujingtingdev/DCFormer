...
class Fusion(data.Dataset):
    def __init__(self, opt, root_path, train=True, MAE=False):
        ...
        self.MAE=MAE
        if self.train:
            self.poses_train, self.poses_train_2d, self.poses_train_2d_crop, self.valid_frame = self.prepare_data(opt.root_path, train=True)
            self.generator = ChunkedGenerator(opt.batchSize // opt.stride, None, self.poses_train,
                                              self.poses_train_2d, self.poses_train_2d_crop, self.valid_frame, chunk_length=self.stride, pad=self.pad,
        ...
        if train == True:
            ...
            for seq in data.keys():
                for cam in data[seq][0].keys():
                    ...
                    valid_frame[(subject_name, seq_name, cam)] = anim["valid"]
    ...
    def __getitem__(self, index):
        ...
        if self.MAE:
            ...
            cam, gt_3D, input_2D, input_2D_crop, img, depth, seq, subject, cam_ind = self.generator.get_batch(seq_name, start_3d, end_3d, flip, reverse)
        ...
        if self.MAE:
            ...
            if self.train == True:
                return cam, gt_3D, input_2D, input_2D_crop, img, depth, seq, subject, scale, bb_box, cam_ind
            else:
                return cam, gt_3D, input_2D, input_2D_crop, img, depth, seq, scale, bb_box
